...
### Kịch bản chi tiết 5 phút:

1. **Phút 1: Bulk Insert & Cơ chế Batching (Cũ vs Mới)**
   - **Thao tác**: Gửi `POST /demo/efcore/seed-products?count=1000&compareLoop=true`
   - **Lời thoại / Giải thích**:
     - *"Cách cũ/ngây thơ"*: Chạy vòng lặp `for` gọi `Add()` và `SaveChanges()` cho từng item. Chỉ 50 items mà mất hàng ngàn ms vì mỗi lần `SaveChanges()` phải mở connection, tạo transaction và gửi round-trip mạng tới PostgreSQL.
     - *"Cách tối ưu"*: Dùng `AddRange()` và gọi `1 lần SaveChangesAsync()` để nạp 1.000 records. EF Core tự động gom các câu lệnh INSERT vào các Batch command, hoàn thành trong tích tắc (~100-200ms).
   - **Kết luận**: Tuyệt đối không gọi `SaveChanges()` bên trong vòng lặp!

2. **Phút 2: Hiệu năng Change Tracker - `Tracking` vs `AsNoTracking`**
   ...
   - **Lời thoại / Giải thích**:
     ...
     - Khi thêm `.AsNoTracking()`: Change Tracker = 0 entities! Tốc độ query nhanh hơn 2x - 4x và RAM hoàn toàn giải phóng vì EF Core bỏ qua bước snapshot.
   - **Kết luận**: Với mọi API Read-only (GET, báo cáo, export, phân trang), **bắt buộc** phải dùng `AsNoTracking()`.

3. **Phút 3: Cơn ác mộng N+1 Queries & Cứu tinh Eager Loading (`.Include`)**
   - **Thao tác**: Gửi `GET /demo/efcore/n-plus-one?orderCount=10` kết hợp chỉ tay vào màn hình Terminal xem SQL log.
   - **Lời thoại / Giải thích**:
     - *"Cách sai (N+1)"*: Lấy 10 Order, sau đó trong code duyệt vòng lặp để load tiếp OrderItems. Khán giả nhìn màn hình terminal sẽ thấy log bắn ra **11 câu SQL riêng rẽ** (1 câu lấy Order + 10 câu lấy Items). Nếu có 1.000 orders thì DB sẽ hứng 1.001 câu truy vấn, làm nghẽn DB!
     - *"Cách chuẩn (Eager Loading)"*: Dùng `.Include(o => o.Items)`. Terminal chỉ hiển thị **ĐÚNG 1 CÂU SQL `LEFT JOIN` duy nhất**.
...
4. **Phút 4: Cơ chế Deferred Execution (Khi nào SQL thực sự gửi đi?)**
   - **Thao tác**: Gửi `GET /demo/efcore/deferred-execution`
   - **Lời thoại / Giải thích**:
     - `IQueryable<T>` chỉ là cây biểu thức (Expression Tree). Dù ta có `.Where()`, `.OrderBy()`, `.Take()` thì EF Core vẫn **chưa** hề gửi bất kỳ tín hiệu nào tới DB.
     - Dùng `.ToQueryString()` để xem trước câu SQL mà EF Core dự định sinh.
     - Chỉ khi gọi phương thức thực thi (Materialization) như `.ToListAsync()`, `.FirstOrDefaultAsync()`, SQL mới thực sự được bắn vào Database.

5. **Phút 5: Tổng kết & Chuyển sang Mini-game Kahoot**
